...
void ofApp::setup()
{
	...
	gui_pheno.setup("Phenotype", "Configuration System", ofGetWidth() * 0.87, ofGetHeight() * 0.8);
	...
	gui_geno.setup("Genotype", "Configuration System", ofGetWidth() * 0.87, ofGetHeight() * 0.65);
	...
	gui_geno.add(gui_REBIRTH_MUTATE_RANGE.setup("Rebirth Mutate Range", "0.2"));
	gui_geno.add(gui_REBIRTH_NUM_SURVIVORS.setup("Rebirth Num Survivors", "3"));
...
void ofApp::phenoButtonPressed() 
{
	...
	rotation_deterioration_speed = stof(str);

	str = gui_SPEED_INCREMENT;
	speed_increment = stof(str);

	str = gui_SPEED_MAXIMUM;
	speed_maximum = stof(str);

	str = gui_SPEED_DETERIORATION;
	speed_deterioration = stof(str);

	str = gui_SPEED_RATIO_FW_BW;
	speed_ratio_fw_bw = stof(str);

	str = gui_MAX_GENERATION_LIFE_SPAN;
	max_generation_life_span = stoi(str);

	str = gui_DRIFT_STRENGTH;
	drift_strength = stoi(str);
}
...
void ofApp::genoButtonPressed()
{
	...
	mutate_range = stof(str);

	str = gui_MIN_FITNESS;
	min_fitness = stof(str);

	str = gui_INIT_FITNESS;
	init_fitness = stof(str);

	str = gui_REBIRTH_MUTATE_RANGE;
	rebirth_mutate_range = stof(str);

	str = gui_REBIRTH_NUM_SURVIVORS;
	rebirth_num_survivors = stof(str);
...
void ofApp::keyReleased(int key){
	...
	if (key == '1' || key == '2' || key == '3' || key == '4' || key == '5')
	{
		...
		if (key == '1') roadIndex = 0;
		if (key == '2') roadIndex = 1;
		if (key == '3') roadIndex = 2;
		if (key == '4') roadIndex = 3;
		if (key == '5') roadIndex = 4;
